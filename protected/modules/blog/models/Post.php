...
class Post extends YModel
{
    ...
    public function offset($count = null)
    {
        $this->getDbCriteria()->mergeWith(
            array(
                'offset' => $count,
    ...
    /**
     * Условие для сортировки по дате создания
     *
     * @param string $typeSort - типо сортировки
     *
     * @return self
     **/
    public function sortByCreateDate($typeSort = 'ASC')
    {
        $this->getDbCriteria()->mergeWith(
            array(
                'order' => $this->getTableAlias() . '.create_date ' . $typeSort,
            )
        );
        return $this;
    }
