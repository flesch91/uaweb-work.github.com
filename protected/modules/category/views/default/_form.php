...
$form = $this->beginWidget('bootstrap.widgets.TbActiveForm', array(
    ...
    <?php if(count($languages) > 1):?>
        <?php echo $form->dropDownListRow($model, 'lang', $languages, array('class' => 'popover-help','empty' => Yii::t('CategoryModule.news', '--выберите--'))); ?>
        <?php if(!$model->isNewRecord):?>
            <?php foreach($languages as $k => $v):?>
                <?php if($k !== $model->lang):?>
                    <?php if(empty($langModels[$k])):?>
                        <a href="<?php echo $this->createUrl('/category/default/create',array('id' => $model->id,'lang'  => $k));?>"><i class="iconflags iconflags-<?php echo $k;?>" title="<?php echo Yii::t('CategoryModule.news','Добавить перевод на {lang} язык',array('{lang}' => $v))?>"></i></a>
                    <?php else:?>
                        <a href="<?php echo $this->createUrl('/category/default/update',array('id' => $langModels[$k]));?>"><i class="iconflags iconflags-<?php echo $k;?>" title="<?php echo Yii::t('CategoryModule.news','Редактировать перевод на {lang} язык',array('{lang}' => $v))?>"></i></a>
                    <?php endif;?>
                <?php endif;?>
            <?php endforeach;?>
        <?php endif;?>
