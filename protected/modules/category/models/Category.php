...
class Category extends YModel
{
    ...
    public function getAllListByAlias($alias,$selfId = false)
    {
        $conditionArray = ($selfId)
            ? array('condition' => 'alias != :alias', 'params' => array(':alias' => $alias))
            : array();
        return $this->cache(Yii::app()->getModule('yupe')->coreCacheTime)->findAll($conditionArray);
    }
